...
export default function SearchResult({hit}){
  ...
  let producing_org = hit.producing_org || hit.asset?.[0].producing_org[0]
  let media_type = hit.media_type || hit.asset?.[0].media_type[0]
  let title = hit.title || hit.asset?.[0].title[0]
  guid = hit.guid
  ...
  // guid = aapbGuid(pb_doc)
  if(pb_doc){
    description = resultDescription(pb_doc.pbcoreDescription)
