...
export default function Mappy(props){
  ...
  Object.values(orgsByState).map((orgsInState) => {
    Object.keys(orgsInState).map((orgId) => {
      let org = orgs[orgId]
      let logo = org.Logo ? <img src={ `https://s3.amazonaws.com/americanarchive.org/org-logos/${org.Logo}` } className="mappy-logo" /> : null
      markers.push(
        <Marker position={ {lat: org.location[0], lng: org.location[1] } } >
          <Popup>
            ...
            { logo }
            <a href="/search?orgsid?????todo" target="_blank" class="btn btn-default btn-sm">View all records</a>
          </Popup>
        </Marker>
      )  
    })
